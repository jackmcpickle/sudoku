...
import { isValidMove, getCandidates, getAffectedPositions } from '@/lib/sudoku/validator'
import type { Grid, CellValue } from '@/types'

const emptyGrid: Grid = Array.from({ length: 9 }, () => Array(9).fill(0) as CellValue[])
...
describe('getCandidates', () => {
  ...
  it('returns all numbers for empty grid', () => {
    expect(getCandidates(emptyGrid, 0, 0)).toEqual([1, 2, 3, 4, 5, 6, 7, 8, 9])
  })
})
