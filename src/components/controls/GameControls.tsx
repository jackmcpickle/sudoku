  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap justify-center gap-2">
        <Button onClick={undo} variant="secondary" disabled={history.length === 0}>Undo</Button>
        <Button onClick={toggleNotesMode} variant={isNotesMode ? 'primary' : 'secondary'} active={isNotesMode}>
          Notes {isNotesMode ? 'ON' : 'OFF'}
        </Button>
        <Button onClick={useHint} variant="secondary" disabled={hintsRemaining <= 0}>Hint</Button>
      </div>
      <p className="text-xs text-center text-slate-500">Keyboard: 1-9 to enter, N for notes, Ctrl+Z to undo</p>
    </div>
  )
}
