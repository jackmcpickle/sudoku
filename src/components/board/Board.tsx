      ...
      >
        {board.map((row, rowIndex) =>
          row.map((cell, colIndex) => {
            ...
            const isHighlighted = selectedCell !== null && !isSelected && (
              selectedCell.row === rowIndex ||
              selectedCell.col === colIndex ||
              (Math.floor(selectedCell.row / 3) === Math.floor(rowIndex / 3) &&
                Math.floor(selectedCell.col / 3) === Math.floor(colIndex / 3))
            )
            const isError = cell.value !== 0 && cell.value !== solution[rowIndex]?.[colIndex] && !cell.isGiven
            const isSameNumber = !isSelected && selectedValue !== 0 && cell.value === selectedValue
            ...
            return (
              <div key={`${rowIndex}-${colIndex}`} className={`${borderRight} ${borderBottom}`}>
                <Cell
                  value={cell.value}
