...
export function UsernameModal({ isOpen, onClose }: UsernameModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existing && existing.visitorId !== useUserStore.getState().visitorId) {
  ...
  if (showResumePrompt) {
    return (
      <Modal isOpen={isOpen} onClose={onClose || (() => {})} title="Resume Session?">
        <div className="space-y-4">
          <p className="text-sm text-slate-400">
            This username is already in use on another device. Resume that session here?
          </p>
          <div className="flex gap-2">
            <Button onClick={handleResume} className="flex-1" disabled={isLoading}>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose || (() => {})} title="Choose a Username">
        ...
        <div>
          <div className="flex gap-2">
            <input
              ...
              disabled={isLoading}
            />
            <Button type="button" variant="secondary" onClick={handleGenerateName} disabled={isLoading}>
              Random
            </Button>
          </div>
