...
function rowContains(grid: Grid, row: number, value: CellValue, excludeCol?: number): boolean {
  for (let c = 0; c < 9; c++) {
...
function colContains(grid: Grid, col: number, value: CellValue, excludeRow?: number): boolean {
  for (let r = 0; r < 9; r++) {
...
function boxContains(grid: Grid, row: number, col: number, value: CellValue): boolean {
...
export function getCandidates(grid: Grid, row: number, col: number): CellValue[] {
